...
// header
if(isset($_POST['borfno'])){$header[] = 'BORF NUMBER';}
...
if($location == 'all'){
	$f325_query = mysqli_query($conn,"SELECT * FROM dbborflist WHERE emaildate BETWEEN '$from' AND '$to'");
}else{
	$f325_query = mysqli_query($conn,"SELECT * FROM dbborflist WHERE location = '$location' AND emaildate BETWEEN '$from' AND '$to'");
}
	...
	while($fetch_f325 = mysqli_fetch_array($f325_query)) {
		...
		$result	= mysqli_query($conn,"SELECT * FROM dbborfraw WHERE f325number = '$f325number' AND status = '$status' ORDER BY id ASC ");
		...
		while($row = mysqli_fetch_array($result))
		{
			...
			if(isset($_POST['borfno'])){$borfno = $fetch_f325['borfnumber'];}else{$borfno = '@';}
			...
			$array_list = array($borfno,$f325number,$mdccode,$description,$category,$brcode,$brname,$dmpireason,$reasoncode,$quantity,$rcvdqty,$expiration,$f325status,$arnumber,$arreason,$preparedby,$issuedby,$emaildate,$f325date,$company,$datesched,$datecleared,$unitcost,$costextended,$location,$process);
